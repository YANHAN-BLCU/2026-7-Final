...
----------------------------加入新指令addrm的操作码-------------
  CONSTANT ADRRM:STD_LOGIC_VECTOR(7 downto 0):="11101000";--NEW ADRRM
  ...
  begin
    ...
    if (Ir13='0')and(C_M='1')then
	  ...
	  -----------------加入新指令addrm的控制指令-------------
		...
		case T_temp is
			when "0110"=>if instru=ADRRM then
					 Comb:="10001000001101010101000001011000";	--PC->AR,PC+1->PC
					end if;
			when "0100"=>if instru=ADRRM then
					 Comb:="00100100011100000000000000011000";	--MEM->AR
					end if;
			when "0111"=>if instru=ADRRM then
					 Comb:="00100000011100000000000000000000";	--MEM->Q
					end if;	
			when "0101"=>if instru=ADRRM then
					 Comb:="100011000000"&Dr&Sr&"001000000000";	--Q+SR->DR
					end if;
			when others=>null;
		end case;
	else null;
