...
--******************C set instruction operating code****************
  ...
  ---------------此处添加新指令操作码ADAA------------------
  CONSTANT ADAA:STD_LOGIC_VECTOR(7 downto 0):="11100001";--new adaa
  ...
  begin
    ...
    if((reset='0')and(MACH='0')AND(C_M='1'))then T_temp<="1000";
         ...
         case T_temp is
		   ...
		   -------------------此处添加新指令ADAA的节拍------------------
		   when "0101"=>if(Ir11='0')               ----扩展指令adaa
							then T_temp<="0001";
							else T_temp<="0000";
						end if;
		   when "0001"=>T_temp<="1001";
		   when "1001"=>T_temp<="1011";
		   when "1011"=>T_temp<="1010";
    ...
    if (Ir13='0')and(C_M='1')then
      case T_temp is
	   ...
	   -------------------此处添加新指令ADAA的控制指令------------------
		...
		case T_temp is
			when "0110"=>if instru=ADAA then
						Comb:="10001000001101010101000001011000";  --PC->AR,PC+1->PC
						end if;
			when "0111"=>if instru=ADAA then
						Comb:="00100100011100000000000000011000";  --MEM->AR
						end if;
			when "0101"=>if instru=ADAA then
						Comb:="00100000011100000000000000000000";  --MEM->Q
						end if;
			when "0001"=>if instru=ADAA then
						Comb:="10001000001101010101000001011000";  ---PC->AR,PC+1->PC
						end if;
			when "1001"=>if instru=ADAA then
						Comb:="00100100011100000000000000011000";  --MEM->AR
						end if;
			when "1011"=>if instru=ADAA then
						Comb:="00100000011000000000000000000000";--MEM+Q->Q
						end if;
			when "1010"=>if instru=ADAA then
						Comb:="00000100001000000000000000000001";--Q->MEM
						end if;
			when others=>null;
		end case;
	else null;
