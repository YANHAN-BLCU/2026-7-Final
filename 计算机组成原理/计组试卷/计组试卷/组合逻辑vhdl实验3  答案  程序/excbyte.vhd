...
-------------------添加新的引脚 用作节拍的计数器----------
	   T_3,T_2,T_1,T_0:buffer std_logic);
...
--******************C set instruction operating code****************
 ...
 ----------------------此处添加新指令exc的操作码---------------
  CONSTANT EXC :STD_LOGIC_VECTOR(7 DOWNTO 0):="00100010";
...
-----------------额外添加一个临时变量op-----------
  variable op:std_logic_vector(7 downto 0):=Ir15&Ir14&Ir13&Ir12&Ir11&Ir10&Ir9&Ir8;  
...
-----------------------新增控制器计数器-----------
  ...
  begin
	if((Clk'event) and (Clk='1'))then
		case T_temp is
			when "0011"=>if(op="00100010")
							then G_T<="0000";
							--else null;
						 end if;
			when "1011"=>if(op="00100010")then
							G_T(0)<=not T_0;
							G_T(1)<=((not T_1) and T_0) or (T_1 and (not T_0));
							G_T(2)<=((not T_2) and T_1 and T_0) or (T_2 and (not (T_1 and T_0)));
							G_T(3)<=((not T_3) and T_2 and T_1 and T_0) or (T_3 and (not (T_2 and T_1 and T_0)));
						 end if;
			when others=>null;	
		end case;
	end if;
	T_3<=G_T(3);T_2<=G_T(2);T_1<=G_T(1);T_0<=G_T(0);
  end process Count_p;
--******************Clock Time generator***********************
  Timing_p:process(Clk,Reset,G_T)
    variable temp:std_logic_vector(1 downto 0);	
  begin
	--op  :=Ir15&Ir14&Ir13&Ir12&Ir11&Ir10&Ir9&Ir8;
    temp:=(Ir14,Ir11);
...
--******************Combined logic generator***********************
  Combin_p: process(T_temp)
	--variable Comb1:std_logic_vector(31 downto 0);
...
--------------------此处添加新指令exc的控制信号---------------
    elsif Ir13='1' then--Extended Instruction
		case T_temp is
			when "0011"=>if instru=EXC then
					 Comb:="100000000011"&Dr&"0000000000000000";--EXC DR->Q
					 end if;
			when "1011"=>if instru=EXC then
					 Comb:="100100000011"&Dr&"0000000111000000";--EXC DR/2->DR,Q/2->Q
					 end if; 
			when "1111"=>if instru=EXC then
					 Comb:="100011000010"&Dr&"0000000000000000";--EXC Q->DR
					 end if; 
			when others=>null;
		end case;
	else null;
